...
.estimate_mpj <- function(mat_lis, idx){
  stopifnot(is.list(mat_lis), is.numeric(idx), all(sapply(mat_lis, nrow) == length(idx)))

  n <- length(mat_lis)
  lis <- lapply(1:n, function(x){.estimate_mpm(mat_lis[[x]], idx)})
